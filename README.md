...
- **DevinLocal 模式**：拦截 `server.codeium.com`，适用于 Devin Desktop，清洗 Cascade 身份指令，工具名归一化到 Devin 风格
- **Cascade 模式**：拦截 `server.self-serve.windsurf.com`，适用于 Windsurf IDE，保留 Cascade 身份和 Windsurf 原生工具名
...
1. **代理模式**：侧栏设置 Tab 选 DevinLocal 或 Cascade。DevinLocal 拦截 `server.codeium.com`（Devin Desktop），Cascade 拦截 `server.self-serve.windsurf.com`（Windsurf IDE）。切换后自动重启代理生效。两种模式都支持子 Agent。
...
- **DevinLocal 模式**：拦截 `server.codeium.com`，清洗 Cascade 身份指令，工具名归一化到 Devin 风格（`read_file` → `read`）
- **Cascade 模式**：拦截 `server.self-serve.windsurf.com`，保留 Cascade 身份和 Windsurf 原生工具名（`read_file` / `grep_search` 等原样回传）
...
- **DevinLocal**（默认）：拦截 `server.codeium.com`，适用于 Devin Desktop。清洗 Cascade 身份指令，工具名归一化到 Devin 风格
- **Cascade**：拦截 `server.self-serve.windsurf.com`，适用于 Windsurf IDE。保留 Cascade 身份和 Windsurf 原生工具名
