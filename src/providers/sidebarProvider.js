...
class SidebarProvider {
  ...
  getHtml() {
    ...
    const tmp34 = tmp1.backupStale ? 'badge-error' : (tmp7 === tmp1.patches.length ? 'badge-ok' : 'badge-warn');
    const tmp35 = tmp1.backupStale ? '客户端已更新' : (tmp7 === tmp1.patches.length ? '已就绪' : '需安装');
    const tmp36 =
