...
function getPatches() {
  if (!patchCache) {
    patchCache = PATCH_RULES.map(rule => ({
      ...
      targetFile: rule.targetFile || "extension"
    }));
  }
  return patchCache;
}
class PatchManager {
  ...
  static findWorkbenchMainJs() {
    const tmp0 = [];
    if (process.platform === "darwin") {
      PatchManager.addCandidate(tmp0, "/Applications/Devin.app/Contents/Resources/app/out/vs/workbench/workbench.desktop.main.js");
    }
    ...
    if (process.platform === "linux") {
      ...
      if (tmp1) {
        for (const tmp02 of [".devin-server"]) {
          const tmp2 = path.join(tmp1, tmp02, "bin");
          if (fs.existsSync(tmp2)) {
            try {
              for (const tmp03 of fs.readdirSync(tmp2)) {
                PatchManager.addCandidate(tmp0, path.join(tmp2, tmp03, "out", "vs", "workbench", "workbench.desktop.main.js"));
              }
            } catch {}
          }
        }
  ...
  static resolveWorkbenchPath() {
    const tmp0 = PatchManager.findWorkbenchMainJs();
    if (tmp0 && fs.existsSync(tmp0)) {
      return tmp0;
    }
    return null;
  }
  ...
  static addWindowsShortcutCandidates(tmp0) {
    ...
    for (const tmp02 of tmp4) {
      ...
      try {
        const tmp03 = (0, child_process_1.execFileSync)("powershell.exe", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", "$shell=New-Object -ComObject WScript.Shell; Get-ChildItem -LiteralPath " + JSON.stringify(tmp02) + " -Recurse -Filter *.lnk -ErrorAction SilentlyContinue | Where-Object { $_.Name -match 'Devin' } | ForEach-Object { $shell.CreateShortcut($_.FullName).TargetPath }"], {
          encoding: "utf8",
          windowsHide: true,
          timeout: 3500
        });
  ...
  static findExtensionJs() {
    ...
    if (process.platform === "win32") {
      ...
      for (const tmp03 of ["C", "D", "E", "F"]) {
        for (const tmp04 of ["Devin", "devin"]) {
          PatchManager.addCandidate(tmp0, path.join(tmp03 + ":", tmp04, "resources", "app", "extensions", "windsurf", "dist", "extension.js"));
        }
      }
    }
    if (process.platform === "linux") {
      const tmp02 = process.env.HOME || "";
      if (tmp02) {
        for (const tmp03 of [".devin-server"]) {
  ...
  static updateChecksum(tmp0) {
    try {
      ...
      if (tmp4.checksums) {
        const baseName = path.basename(tmp0);
        const tmp03 = Object.keys(tmp4.checksums).find(arg0 => arg0.endsWith(baseName));
        if (tmp03) {
  ...
  static applyPatchesToFile(tmp0, tmp1, tmp2, tmp3) {
    ...
    if (!tmp5) {
      try {
        const cur = fs.readFileSync(tmp0, "utf-8");
        const dirtyMarkers = ["MODEL_CUSTOM_BYOK", "availableModels", "configOptions"];
        const isDirty = getPatches().some(arg0 => (arg0.targetFile || "extension") === tmp3 && PatchManager.isPatched(cur, arg0, tmp1, tmp2)) || (tmp3 === "workbench" && dirtyMarkers.some(m => cur.includes(m + ".map") || cur.includes(m + ".concat")));
        if (isDirty) {
          console.log("[patchManager] ⚠️ 当前文件已被 patch 但无干净备份，跳过备份创建避免脏文件当原版: " + tmp0);
        } else {
          fs.copyFileSync(tmp0, tmp4);
        }
      } catch {}
    }
    let tmp6 = fs.readFileSync(tmp0, "utf-8");
    const tmp7 = tmp5 ? fs.readFileSync(tmp5, "utf-8") : fs.existsSync(tmp4) ? fs.readFileSync(tmp4, "utf-8") : null;
    const tmp8 = getPatches().filter(arg0 => (arg0.targetFile || "extension") === tmp3).some(arg0 => PatchManager.isPatched(tmp6, arg0, tmp1, tmp2));
    if (tmp8 && tmp7) {
      tmp6 = tmp7;
    }
    const tmp9 = [];
    ...
    for (const tmp02 of getPatches()) {
      if ((tmp02.targetFile || "extension") !== tmp3) {
        continue;
      }
      if (PatchManager.isPatched(tmp6, tmp02, tmp1, tmp2)) {
        tmp11++;
      ...
      const tmp03 = PatchManager.applyPatchContent(tmp6, tmp02, tmp1, tmp2);
      if (!tmp03.changed) {
        tmp12++;
  ...
  static applyWithCustomUrls(tmp0, tmp1, tmp2) {
    ...
    const tmp4 = PatchManager.applyPatchesToFile(tmp3, tmp0, tmp1, "extension");
    const tmp5 = PatchManager.resolveWorkbenchPath();
    let tmp6 = {applied: 0, skipped: 0, failed: 0, details: []};
    if (tmp5) {
      tmp6 = PatchManager.applyPatchesToFile(tmp5, tmp0, tmp1, "workbench");
    } else {
      tmp6.details.push("[缺失] workbench.desktop.main.js (Devin Local 模型注入跳过)");
    }
    const tmp7 = tmp4.failed + tmp6.failed;
    const tmp8 = tmp4.applied + tmp6.applied;
    const tmp9 = tmp4.skipped + tmp6.skipped;
    const tmp10 = [...tmp4.details, ...tmp6.details];
    const tmp11 = {
      success: tmp7 === 0,
      applied: tmp8,
      skipped: tmp9,
