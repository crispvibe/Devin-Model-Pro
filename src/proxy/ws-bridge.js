...
export function startWSBridge(arg0) {
  arg0.on("upgrade", (arg02, arg1, arg2) => {
    ...
    arg1.on("error", arg03 => {
      clients.delete(arg1);
      if (arg03.code === "EPIPE" || arg03.code === "ECONNRESET" || /EPIPE|ECONNRESET/i.test(arg03.message)) {
        return;
      }
...
// ===== 子 Agent 事件（BYOK 本地执行，推到 sidebar 监控面板）=====
// agentId 由 subagent-executor 生成（sub-<ts>-<rand>），同一子 Agent 全程复用
export function emitSubagentStart(arg0) {
  broadcast({
    type: "subagent_start",
    ts: Date.now(),
    targetId: monitorTarget(arg0.targetId),
    agentId: arg0.agentId,
    profile: arg0.profile || "subagent_general",
    task: (arg0.task || "").slice(0, 500),
    model: arg0.model || "",
    provider: arg0.provider || "",
    toolCount: arg0.toolCount || 0
  });
}
export function emitSubagentTurn(arg0) {
  broadcast({
    type: "subagent_turn",
    ts: Date.now(),
    targetId: monitorTarget(arg0.targetId),
    ...
    turn: arg0.turn || 0,
    tools: arg0.tools || [],
    text: (arg0.text || "").slice(0, 3000)
  });
}
export function emitSubagentToolExec(arg0) {
  broadcast({
    type: "subagent_tool_exec",
    ts: Date.now(),
    targetId: monitorTarget(arg0.targetId),
    agentId: arg0.agentId,
    tool: arg0.tool || "",
    args: (arg0.args || "").slice(0, 50000),
    callId: arg0.callId || ""
  });
}
export function emitSubagentToolResult(arg0) {
  broadcast({
    ...
    callId: arg0.callId || "",
    tool: arg0.tool || "",
    result: (arg0.result || "").slice(0, 50000),
    ok: arg0.ok !== false
  });
}
export function emitSubagentEnd(arg0) {
  broadcast({
    type: "subagent_end",
    ts: Date.now(),
    targetId: monitorTarget(arg0.targetId),
    agentId: arg0.agentId,
    ok: arg0.ok !== false,
    summary: (arg0.summary || "").slice(0, 3000),
    turns: arg0.turns || 0,
    reason: arg0.reason || "done"
  });
}
