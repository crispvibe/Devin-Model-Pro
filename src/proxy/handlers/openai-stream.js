...
import { buildTextDelta, buildThinkingDelta, buildToolCallDelta, buildStopChunk, buildErrorChunk, createChunkContext, STOP_REASON } from "./build-response.js";
...
export class OpenAIStreamProcessor {
  constructor(tmp0, tmp1, tmp2 = null, tmp3 = null) {
    this._messageId = tmp0;
    this._modelUid = tmp1;
    this._targetId = tmp2;
    this._chunkCtx = tmp3 || createChunkContext({
      messageId: tmp0,
      modelUid: tmp1,
      startTime: Date.now()
    });
  ...
  _onDone() {
    ...
    if (this._errorMessage) {
      ...
      tmp0.push(buildErrorChunk(this._messageId, "[OpenAI Error] " + this._errorMessage, this._chunkCtx));
      this._done = true;
      return tmp0;
    }
    ...
    if (tmp2.length > 0) {
      ...
      if (tmp02.length > 0) {
        tmp1.push(...tmp02);
        tmp0.push(buildToolCallDelta(this._messageId, tmp02, this._chunkCtx));
        this._stopReason = "tool_calls";
      } else {
      ...
      if (tmp12.length > 0) {
        ...
        tmp1.push(...tmp03);
        tmp0.push(buildToolCallDelta(this._messageId, tmp03, this._chunkCtx));
        this._stopReason = "tool_calls";
      } else {
        this._restoreInterceptedText(tmp0);
      }
    ...
    tmp0.push(buildStopChunk(this._messageId, tmp4, this._modelUid, this._chunkCtx));
    this._done = true;
    if (tmp1.length > 0) {
  ...
  _emitTextChunk(tmp0, tmp1) {
    ...
    this._tokenCount++;
    tmp1.push(buildTextDelta(this._messageId, tmp0, this._tokenCount, this._chunkCtx));
...
export class ChatCompletionsStreamProcessor {
  ...
  processEvent(tmp0) {
    ...
    if (tmp4.reasoning_content) {
      tmp3.push(buildThinkingDelta(this._messageId, tmp4.reasoning_content, this._chunkCtx));
    }
    if (tmp4.content) {
