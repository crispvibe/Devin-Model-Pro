...
import { buildErrorChunk, buildTextDelta, buildMetadataChunk, buildStatsChunk, createChunkContext } from './build-response.js';
...
import { runSubagent, findFailedSubagents } from './subagent-executor.js';
import { clearSubagents } from '../subagent-state.js';

// run_subagent 本地执行结果缓存：tool_use_id -> result
// 同一 tool_use 只本地执行一次，后续请求命中缓存直接替换，避免每轮重跑
const subagentResultCache = new Map();
const SUBAGENT_CACHE_MAX = 50;
...
function writeModelConfigError(arg0, arg1, arg2, acceptGzip = true) {
  arg0.writeHead(200, streamHeaders(acceptGzip));
  arg0.write(wrapEnvelope(buildErrorChunk(arg1, arg2), acceptGzip));
  arg0.write(endOfStreamEnvelope(acceptGzip));
  arg0.end();
}
...
function createTimingTracker(arg0, tmp1 = {}, tmp2 = null) {
  ...
  const _timingDebug = process.env.BYOK_DEBUG_TIMING === '1';
  ...
  const tmp8 = (arg02, tmp12 = '') => {
    ...
    if (_timingDebug) {
      ...
      console.log('  ✅ ' + arg0 + ': total=' + tmp22 + 'ms' + (tmp12 ? ' ' + tmp12 : ''));
    }
    emitStreamStatus(
      'timing',
...
const tmp0 = {
  ...
  MODEL_SWE_1_6: ANTHROPIC_FALLBACK_MODEL,
  'swe-1-6': '__DEFAULT__',
  'swe-1-6-medium': '__DEFAULT__',
...
export async function handleGetChatMessage(arg0, arg1, arg2) {
  let {
    systemPrompt: tmp3,
    messages: tmp4,
    tools: tmp5,
  ...
  if (requiresConfiguredDefaultModel(tmp7)) {
    ...
    writeModelConfigError(arg1, tmp9, tmp02, acceptGzip);
    return;
  }
  ...
  if (!requiredKey) {
    ...
    arg1.writeHead(200, streamHeaders(acceptGzip));
    arg1.write(wrapEnvelope(buildErrorChunk(tmp1, 'No ' + tmp02 + ' API key configured'), acceptGzip));
    arg1.write(endOfStreamEnvelope(acceptGzip));
    arg1.end();
    return;
  }
  ...
  if (tmp18.length > 0) {
    ...
    console.log('  📨 Injected: ' + tmp18.length + ' msg');
  }
  ...
  // BYOK 本地子 Agent：拦截失败的 run_subagent tool_result，本地重跑（带缓存，同一 tool_use 只跑一次）
  if (process.env.BYOK_SUBAGENT_ENABLED !== 'false') {
    const failedSubs = findFailedSubagents(tmp4);
    if (failedSubs.length > 0) {
      const toRun = failedSubs.filter(fs => !subagentResultCache.has(fs.toolUse.id));
      const cached = failedSubs.filter(fs => subagentResultCache.has(fs.toolUse.id));
      // 命中缓存的直接替换
      for (const fs of cached) {
        fs.toolResult.content = subagentResultCache.get(fs.toolUse.id);
  ...
  if (tmp4.length > 0) {
    const tmp02 = tmp4.map((arg02) => arg02.role);
    const roleSummary = tmp02.length <= 6
      ? tmp02.join(',')
      : tmp02.slice(0, 3).join(',') + ',...(' + (tmp02.length - 4) + '),...' + tmp02.slice(-1)[0];
...
function createStreamLifecycle(arg0, fn, arg2, arg3, arg4, acceptGzip = true) {
  let tmp5 = false;
  let tmp6 = false;
  let tmp7 = null;
  let tmp8 = Date.now();
  ...
  const tmp10 = () => {
    ...
    tmp7 = setInterval(() => {
      ...
      if (Date.now() - tmp8 >= tmp9) {
        arg0.write(wrapEnvelope(buildTextDelta(arg3, '', 0), acceptGzip));
      }
    }, tmp9);
  };
  const fn2 = () => {
  ...
  const fn4 = (arg02) => {
    ...
    tmp6 = true;
    fn2();
    fn3(endOfStreamEnvelope(acceptGzip));
    arg0.end();
    if (arg02) {
      console.log(arg02);
    }
  ...
  const tmp14 = (arg02, arg1) => {
    ...
    if (arg02) {
      fn3(wrapEnvelope(buildErrorChunk(arg3, arg02), acceptGzip));
    }
    return fn4(arg1);
  };
  arg0.on('close', () => {
...
function buildOpenAIResponsesBody({
  ...
  const tmp20 = tmpGenConfig?.max_tokens || getMaxTokens();
  if (tmp20 > 0) {
    tmp19.max_output_tokens = tmp20;
  }
  if (tmpGenConfig?.temperature != null) {
    tmp19.temperature = tmpGenConfig.temperature;
  }
  if (tmpGenConfig?.top_p != null) {
    tmp19.top_p = tmpGenConfig.top_p;
  }
  ...
  function processPart(arg03) {
    ...
    for (const tmp02 of tmp110) {
      ...
      for (const tmp04 of tmp03) {
        tmp24.safeWrite(wrapEnvelope(tmp04, acceptGzip));
      }
    }
    if (tmp13.isDone) {
  ...
  arg02.on('end', () => {
    ...
    if (!tmp13.isDone && tmp11 && !tmp11.writableEnded) {
      ...
      for (const tmp03 of tmp02) {
        tmp24.safeWrite(wrapEnvelope(tmp03, acceptGzip));
      }
    }
...
function streamAnthropic(
  ...
  const tmp14 = {
    ...
    stream: true,
    max_tokens: tmpGenConfig?.max_tokens || getMaxTokens(),
  };
  if (tmpGenConfig?.temperature != null) {
    tmp14.temperature = tmpGenConfig.temperature;
  }
  if (tmpGenConfig?.top_p != null) {
    tmp14.top_p = tmpGenConfig.top_p;
  }
  ...
  if (tmp10?.thinkingEnabled) {
    ...
    if (tmp02?.thinking) {
      ...
      if (tmp03 > 0 && tmp14.max_tokens <= tmp03) {
        tmp14.max_tokens = Math.min(tmpGenConfig?.max_tokens || getMaxTokens(), tmp03 + 8192);
      }
    }
  }
  ...
  const chunkCtx = createChunkContext({
    messageId: tmp7,
    modelUid: tmp6,
    agentType: tmp6,
    chatcmplId: tmp7.startsWith('bot-') ? tmp7.slice(4) : tmp7,
    startTime: Date.now()
  });
  const processor = new AnthropicStreamProcessor(tmp7, tmp6, tmp9, chunkCtx);
  let tmp17;
  const tmp18 = createStreamLifecycle(arg1, () => tmp17, 'Anthropic', tmp7, tmp8, acceptGzip);
  // 开头 metadata chunk（真实 Devin 响应在流开始时发送）
  // BYOK_SKIP_METADATA=true 时跳过，用于排查 metadata chunk 是否导致客户端断开
  if (process.env.BYOK_SKIP_METADATA !== 'true') {
    const tmpMeta = buildMetadataChunk(chunkCtx);
    if (tmpMeta) {
      tmp18.safeWrite(wrapEnvelope(tmpMeta, acceptGzip));
    }
  }
    ...
    (arg02) => {
      ...
      function processPart(arg03) {
        ...
        for (const tmp02 of tmp110) {
          ...
          for (const tmp04 of tmp03) {
            tmp18.safeWrite(wrapEnvelope(tmp04, acceptGzip));
          }
        }
      ...
      arg02.on('end', () => {
        ...
        if (!processor.isDone && !arg1.writableEnded) {
          ...
          for (const tmp03 of tmp02) {
            tmp18.safeWrite(wrapEnvelope(tmp03, acceptGzip));
          }
          logAnthropicUsage();
...
function streamOpenAI(
  ...
  if (tmp16 && tmp4 && tmp4.length > 0) {
    console.log(
      '  🔧 OpenAI tools: ' + tmp4.length + ' (initiator=' + (tmp9 || 'unknown') + ')'
    );
  ...
  let tmp23;
  let processor;
  const tmp24 = createStreamLifecycle(arg1, () => tmp23, 'OpenAI', tmp8, tmp10, acceptGzip);
  let tmp25 = false;
  let tmp34 = 0;
  let tmp35 = '';
  const fn = (arg02) => {
  ...
  const fn2 = (retryCount = 0, lastError = null) => {
    ...
    const chunkCtx = createChunkContext({
      ...
      modelUid: tmp6,
      agentType: tmp6,
      chatcmplId: tmp8.startsWith('bot-') ? tmp8.slice(4) : tmp8,
      startTime: Date.now()
    });
    processor =
      tmp02.mode === 'chat'
        ? new ChatCompletionsStreamProcessor(tmp8, tmp6, tmp11, chunkCtx)
        : new OpenAIStreamProcessor(tmp8, tmp6, tmp11, chunkCtx);
    if (tmp16 && tmp4) {
    ...
    if (process.env.BYOK_SKIP_METADATA !== 'true') {
      const tmpMeta = buildMetadataChunk(chunkCtx);
      if (tmpMeta) {
        tmp24.safeWrite(wrapEnvelope(tmpMeta, acceptGzip));
      }
    }
