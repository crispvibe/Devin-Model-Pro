...
const SYSTEM_PROMPT_DISABLE_DEFAULT = process.env.SYSTEM_PROMPT_DISABLE_DEFAULT === "true";
const DEVIN_GLOBAL_RULES_PATH = path.join(os.homedir(), ".codeium", "windsurf", "memories", "global_rules.md");
...
function getCustomSystemPrompt() {
  // 优先级 1：显式配置的 SYSTEM_PROMPT_OVERRIDE + SYSTEM_PROMPT_PATH
  if (SYSTEM_PROMPT_OVERRIDE && SYSTEM_PROMPT_PATH) {
    try {
      ...
      _promptCache = { content: tmp1, mtime: tmp0.mtimeMs, path: SYSTEM_PROMPT_PATH };
  ...
  // 优先级 2：默认读取 Devin 全局规则（除非显式禁用）
  if (SYSTEM_PROMPT_DISABLE_DEFAULT) {
    return "";
  }
  try {
    if (!fs.existsSync(DEVIN_GLOBAL_RULES_PATH)) {
      return "";
    }
    const tmp0 = fs.statSync(DEVIN_GLOBAL_RULES_PATH);
    if (_promptCache.path === DEVIN_GLOBAL_RULES_PATH && _promptCache.mtime === tmp0.mtimeMs) {
      return _promptCache.content;
    }
    const tmp1 = fs.readFileSync(DEVIN_GLOBAL_RULES_PATH, "utf8").trim();
    if (!tmp1) {
      return "";
    }
    _promptCache = { content: tmp1, mtime: tmp0.mtimeMs, path: DEVIN_GLOBAL_RULES_PATH };
    console.log("  📝 Devin global rules loaded as system prompt (" + tmp1.length + " chars)");
    return tmp1;
  } catch (tmp0) {
    console.error("  ❌ Failed to load Devin global rules: " + tmp0.message);
    return "";
  }
}
...
function sanitizeSystemPromptIdentity(prompt) {
  if (!prompt) return "";
  let cleaned = prompt;
  cleaned = cleaned.replace(/-?\s*If asked who you are or what model you are[^\n]*\n?/gi, "");
  cleaned = cleaned.replace(/You are Cascade[^\n]*\n?/gi, "");
  cleaned = cleaned.replace(/driven by Cognition's SWE-[0-9.]+[^\n]*\n?/gi, "");
  cleaned = cleaned.replace(/answer\s+`?Cascade`?[^\n]*\n?/gi, "");
  return cleaned;
}
...
function warnUnsignedThinkingStripped() {
  if (!_warnedUnsignedThinking) {
    ...
    console.log("  ℹ️  Stripped unsigned Claude thinking block before forwarding. Set STRIP_UNSIGNED_THINKING=false to preserve it.");
  }
}
...
export function parseGetChatMessageRequest(arg0, arg1) {
  ...
  // 解析 generation config（field 8）：f2=max_tokens, f5=temperature(LE double), f6=top_p(LE double), f7=top_k, f9=stop sequences
  const tmpGenCfg = getField(tmp3, 8, 2);
  let tmpGenConfig = null;
  if (tmpGenCfg) {
    const tmpGc = parseFields(tmpGenCfg.value);
    const tmpGcMaxTokens = getField(tmpGc, 2, 0);
    const tmpGcTemp = getField(tmpGc, 5, 1);
    const tmpGcTopP = getField(tmpGc, 6, 1);
    const tmpGcTopK = getField(tmpGc, 7, 0);
    const tmpGcStop = getAllFields(tmpGc, 9, 2).map(arg0 => arg0.value.toString("utf8"));
    tmpGenConfig = {
      max_tokens: tmpGcMaxTokens ? tmpGcMaxTokens.value : null,
      temperature: tmpGcTemp ? Buffer.from(tmpGcTemp.value).readDoubleLE() : null,
      top_p: tmpGcTopP ? Buffer.from(tmpGcTopP.value).readDoubleLE() : null,
      top_k: tmpGcTopK ? tmpGcTopK.value : null,
      stop: tmpGcStop.length > 0 ? tmpGcStop : null
    };
  }
