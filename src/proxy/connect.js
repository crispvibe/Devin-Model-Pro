...
export function endOfStreamEnvelope(compress = true) {
  if (compress) {
  ...
  const tmp1 = Buffer.alloc(5);
  tmp1[0] = 2;
  tmp1.writeUInt32BE(2, 1);
  return Buffer.concat([tmp1, Buffer.from("{}")]);
}
...
export function wrapUnary(arg0, compress = true) {
  if (compress) {
    return gzipSync(arg0);
  }
  return arg0;
}
export function unaryHeaders(acceptGzip = true) {
  if (acceptGzip) {
    return {
...
export function streamHeaders(acceptGzip = true) {
  if (acceptGzip) {
    return {
