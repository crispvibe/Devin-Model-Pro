...
import { parseFields, getField, getAllFields, fieldToString, fieldToInt, writeStringField, writeBytesField, writeVarintField, writeFixed64Field, writeFixed32Field, writeMessageField, decodeVarint } from "./proto.js";
...
const MITM_CERTS = {};
function loadMitmCert(host) {
  try {
    const certPath = CERTS_DIR instanceof URL ? new URL(host + ".pem", CERTS_DIR) : CERTS_DIR + "/" + host + ".pem";
    const keyPath = CERTS_DIR instanceof URL ? new URL(host + "-key.pem", CERTS_DIR) : CERTS_DIR + "/" + host + "-key.pem";
    MITM_CERTS[host] = { cert: fs.readFileSync(certPath), key: fs.readFileSync(keyPath) };
    console.log("✅ MITM certs loaded for " + host + " from " + (CERTS_DIR instanceof URL ? CERTS_DIR.href : CERTS_DIR));
  } catch {
    console.log("⚠️  No MITM certs found for " + host + " — CONNECT MITM disabled for this host");
  }
}
loadMitmCert(REAL_CODEIUM_HOST);
let MITM_CERT = MITM_CERTS[CERT_HOST]?.cert;
let MITM_KEY = MITM_CERTS[CERT_HOST]?.key;
...
const STREAMING_METHODS = new Set(["GetStreamingCompletions", "GetStreamingExternalChatCompletions", "GetChatMessage"]);
function makeRealResponseTee(arg3) {
  if (process.env.REAL_RESPONSE_TEE !== "true") {
  ...
  let txtFd = null;
  try {
    binFd = fs.openSync(binPath, "w");
    txtFd = fs.openSync(txtPath, "w");
  } catch (e) {
    console.error("  [#" + arg3 + "] 📦 real-response tee open failed: " + e.message);
  }
  console.log("  [#" + arg3 + "] 📦 real-response tee → " + binPath + " / " + txtPath);
  let buf = Buffer.alloc(0);
  let envIdx = 0;
  const logLine = (s) => {
    if (txtFd !== null) {
      try { fs.appendFileSync(txtFd, s + "\n"); } catch {}
    }
    console.log("  [#" + arg3 + "] 📦 " + s);
  };
  const parseEnvelopes = () => {
    while (buf.length >= 5) {
      const flag = buf[0];
      const len = buf.readUInt32BE(1);
      if (buf.length < 5 + len) break;
      const raw = buf.subarray(0, 5 + len);
      const payload = buf.subarray(5, 5 + len);
      if (binFd !== null) {
        try { fs.appendFileSync(binFd, raw); } catch {}
      }
      let proto = payload;
      if (flag === 1) {
        const gz = tryGunzip(payload);
        if (gz) proto = gz;
      }
      const fields = parseFields(proto);
      logLine("envelope[" + envIdx + "] flag=" + flag + " len=" + len + " protoLen=" + proto.length + " fields=" + fields.length);
      for (const f of fields) {
        let valDesc;
        if (f.wireType === 0) valDesc = "varint=" + f.value;
        else if (f.wireType === 1) valDesc = "fixed64(" + f.value.length + "b)";
        else if (f.wireType === 2) valDesc = "bytes(" + f.value.length + "b)";
        else if (f.wireType === 5) valDesc = "fixed32(" + f.value.length + "b)";
        else valDesc = "wire" + f.wireType;
        logLine("  field=" + f.field + " wireType=" + f.wireType + " " + valDesc);
      }
      envIdx++;
  ...
  const tee = new Transform({
    transform(chunk, enc, cb) {
      cb(null, chunk);
      if (binFd === null) return;
      buf = Buffer.concat([buf, chunk]);
      try { parseEnvelopes(); } catch (e) {
        logLine("parse error: " + e.message);
      }
    },
    flush(cb) {
      if (buf.length > 0) {
        logLine("trailing bytes: " + buf.length + " (incomplete envelope)");
        if (binFd !== null) {
          try { fs.appendFileSync(binFd, buf); } catch {}
        }
      }
      logLine("done envelopes=" + envIdx);
      try { if (binFd !== null) fs.closeSync(binFd); } catch {}
      try { if (txtFd !== null) fs.closeSync(txtFd); } catch {}
      cb();
    }
  });
  return tee;
...
function proxyToCodeium(arg0, arg1, arg2, arg3, tmp4 = {}) {
  ...
  const tmp6 = getUpstreamHost(arg0.url, tmp4.forceOfficial === true, arg0.headers.host || "");
  ...
  tmp10.host = tmp6;
  const tmp11 = tmp4.forceOfficial === true ? {} : signUpstreamRequest(arg0.method, tmp7, arg2);
  ...
  const tmp13 = https.request(tmp12, arg02 => {
    if (tmp8) {
      ...
      if (tmp4.forceOfficial === true && tmp5 === "GetChatMessage") {
        const tee = makeRealResponseTee(arg3);
        arg02.pipe(tee).pipe(arg1);
      } else {
        arg02.pipe(arg1);
      }
      arg02.on("end", () => {
        console.log("  [#" + arg3 + "] ✓ streaming done " + tmp5);
      });
      arg02.on("error", () => {
      ...
      arg02.on("error", arg03 => {
        if (!/EPIPE|ECONNRESET|disconnected/i.test(arg03.message)) {
  ...
  tmp13.on("error", arg02 => {
    if (!/EPIPE|ECONNRESET|disconnected/i.test(arg02.message)) {
      console.error("  [#" + arg3 + "] ✗ upstream: " + arg02.message);
    } else {
      console.log("  [#" + arg3 + "] ℹ️ upstream closed: " + arg02.message);
    }
    if (!arg1.headersSent) {
      arg1.writeHead(502);
    }
...
function routeRequest(arg0, arg1, arg2, arg3, tmp4 = "") {
  ...
  if (tmp5 === "GetChatMessage") {
    let isByok = false;
    try {
      const unwrapped = unwrapRequest(arg2, arg0.headers);
      ...
      proxyToCodeium(arg0, arg1, arg2, arg3, { forceOfficial: true });
      return true;
    }
    if (!isByok) {
      console.log("[" + now() + "] #" + arg3 + " ⚡ " + tmp4 + "GetChatMessage → official passthrough");
      proxyToCodeium(arg0, arg1, arg2, arg3, { forceOfficial: true });
      return true;
    }
    console.log("[" + now() + "] #" + arg3 + " ⚡ " + tmp4 + "GetChatMessage → BYOK");
    safeHandle(() => handleGetChatMessage(arg0, arg1, arg2), arg0, arg1, arg3, "Chat");
    return true;
  }
  if (tmp5 === "GetCommandModelConfigs" || tmp5 === "GetCliModelConfigs" || tmp5 === "GetCliTeamSettings" || tmp5 === "GetUserStatus") {
    if (process.env.BYOK_DEBUG_RPC === "1") {
      console.log("[" + now() + "] #" + arg3 + " 📋 " + tmp5 + " → official passthrough");
    }
    proxyToCodeium(arg0, arg1, arg2, arg3, { forceOfficial: true });
    return true;
  }
...
function handleRequest(arg0, arg1) {
  ...
  arg0.on("end", () => {
    ...
    if (arg0.url === "/api/subagents" && arg0.method === "GET") {
      arg1.writeHead(200, {
        "content-type": "application/json",
        "access-control-allow-origin": "*",
        "cache-control": "no-store"
      });
      arg1.end(JSON.stringify({ agents: getSubagentList() }));
      return;
    }
    if (arg0.url === "/api/subagents/clear" && arg0.method === "POST") {
      clearSubagents();
      arg1.writeHead(200, {
        "content-type": "application/json",
        "access-control-allow-origin": "*"
    ...
    proxyToCodeium(arg0, arg1, tmp02, tmp2, { forceOfficial: true });
  });
}
...
const mitmServer = http.createServer((arg0, arg1) => {
  ...
  arg0.on("end", () => {
    ...
    if (process.env.BYOK_DEBUG_MITM === "1") {
...
function attachHybridConnectHandler(arg0) {
  arg0.on("connect", (arg02, arg1, arg2) => {
    ...
    // 对所有已加载 MITM 证书的 host 做 MITM
    // server.codeium.com: devin-cli 通过 NODE_EXTRA_CA_CERTS 信任根 CA
    const mitmC = MITM_CERTS[tmp4];
    if (mitmC) {
      if (process.env.BYOK_DEBUG_MITM === "1") {
